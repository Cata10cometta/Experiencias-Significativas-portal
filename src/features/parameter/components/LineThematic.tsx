...
const LineThematicList: React.FC = () => {
  ...
  const filtered = lineThematics.filter((lt) => {
    ...
    if (!q) return true;
    return (`${lt.code || ''} ${lt.name || ''}`).toLowerCase().includes(q);
  });
  ...
  const handleDeactivate = async (id: number) => {
    ...
    try {
      ...
      setModal({ open: true, type: 'success', message: 'Línea temática desactivada correctamente.' });
      fetchLineThematics(); // Refrescar lista
    } catch (err) {
      setModal({ open: true, type: 'error', message: 'Error al desactivar línea temática.' });
  ...
  const handleActivate = async (id: number) => {
    ...
    try {
      ...
      setModal({ open: true, type: 'success', message: 'Línea temática activada correctamente.' });
      fetchLineThematics(); // Refrescar lista
    } catch (err) {
      setModal({ open: true, type: 'error', message: 'Error al activar línea temática.' });
      console.error("Error al activar línea temática:", err);
    }
  };

  return (
    <div className="w-full max-w-full mx-auto mt-6 px-2 sm:px-6 py-4 sm:py-6 parameter-lines-layout">
      <Joyride
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-2 sm:p-6">
        ...
        <div className="mb-6 flex flex-col sm:flex-row items-center gap-4 parameter-lines-toolbar">
          ...
          <div className="flex gap-4 parameter-lines-filters w-full sm:w-auto">
            <button
        ...
        <div className="overflow-x-auto overflow-y-auto bg-white rounded-2xl border border-gray-200 shadow-sm p-2 sm:p-4 max-h-[60vh] parameter-lines-table">
          <table className="min-w-[600px] w-full rounded-lg overflow-hidden text-sm sm:text-base">
            ...
            <tbody className="text-sm sm:text-base text-gray-700">
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={4} className="py-4 px-3 text-center text-gray-500">No hay líneas temáticas para mostrar.</td>
                </tr>
              ) : (
                paginated.map((lineThematic) => (
                  <tr key={lineThematic.id} className="border-b last:border-b-0 hover:bg-gray-50">
                    <td className="py-2 px-3 break-words max-w-xs">{lineThematic.code}</td>
                    <td className="py-2 px-3 break-words max-w-xs">{lineThematic.name}</td>
                    ...
                    <td className="py-2 px-3">
                      <div className="flex items-center gap-2 sm:gap-3">
                        <button className="text-gray-400 hover:text-sky-600 p-1 sm:p-1.5" style={{minWidth:24}} onClick={() => setEditLineThematic(lineThematic)} title="Editar">
                        ...
                        {lineThematic.state ? (
                          <button className="text-red-400 hover:text-red-600 p-1 sm:p-1.5" style={{minWidth:24}} onClick={() => handleDeactivate(lineThematic.id)} title="Desactivar">
                          ...
                          <button className="text-emerald-500 hover:text-emerald-600 p-1 sm:p-1.5" style={{minWidth:24}} onClick={() => handleActivate(lineThematic.id)} title="Activar">
        ...
        <div className="mt-6 flex flex-col sm:flex-row items-center justify-between gap-2 parameter-lines-pagination">
          <div className="text-sm text-gray-500">
            {filtered.length === 0 ? (
              ...
              (() => {
                ...
                return <>Mostrando {start}-{end} de {filtered.length} líneas temáticas</>;
              })()
            )}
          </div>
